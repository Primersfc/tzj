						...
						if(data==1){
							...
							if($.isArray(data)){
								for(i in data){
									...
									html += '<div class="caption"><div class="stickyContent"><h3 class="stickyTitle">['+ item.leixing  +']<a href="index.php/Index/Article/index/articleid/' + item.article_id + '">' + item.biaoti + '</a></h3></div>';
									html += '<div style="height: 15px;"><div style="float:left;">' + item.diqu + '|' + item.fbsj  + '</div><div style="float:right;"><a href="#" class=" l">' + item.name + '</a></div></div></div>';
									if(item.jiage != ''){
...
function fleshVerify(type){ 
	...
	if (type){
		$('#verifyImg').attr("src", 'index.php/login/verify/adv/1/'+timenow);
	}else{
		$('#verifyImg').attr("src", 'index.php/login/verify/'+timenow);
	}
}
