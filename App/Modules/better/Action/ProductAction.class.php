...
class ProductAction extends CommonAction{
	...
	public function _filter(&$map){
		...
		if(!empty($_REQUEST['keyword'])){
			$map['biaoti']=array("like","%{$_REQUEST['keyword']}%");
		}
	}	
	
	public function add(){
        $m=M('category');
		$clist = $m->where("pid=0")->order('sort desc')->select();
	...
	public function insert(){
		...
		if (false === $model->create()) {
         ...
         $model->type =1;
         $nopic="/sysimg/nophoto.jpg";
	     preg_match("/<img(.*)src=\"([^\"]+)\"[^>]+>/isU",$_POST['content'],$matches);
         $model->imgs=$matches[2];
         if(time()-$_SESSION[C('USER_AUTH_KEY_time')]<20){
			$this->error(L('对不起！公告信息太快了，请等待20秒钟后再发！'));
			}
		if ($result = $model->add()){
	...
	public function edit() {
		...
		$data = $model->find($id);
        $clist = M('category')->where("pid=0")->order('sort desc')->select();
        $clist2 = M('category')->where('pid='.$data['class1'].'')->order('sort desc')->select();
        $oldc1= M('category')->where('id='.$data['class1'].'')->find();
        $oldc2= M('category')->where('id='.$data['class2'].'')->find();
		$this->assign('clist',$clist);
...
public function pushnull(){
   ...
   public function recAll(){
	...
	protected function _list($model, $map = array(), $sortBy = '', $asc = false) {
		...
		//分页查询数据
		$list = $model->where($map)->relation(true)->order('ispush desc,'.$order.' '.$sort)//被推荐的排在前面
